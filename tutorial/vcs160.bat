...
echo Generating project files for Visual Studio 2019 .NET C# ...
...
for %%i in ( *.cs ) do (
	call :add %%~ni
	mkdir vcs160\%%~ni
	call :csproj %%~ni > vcs160\%%~ni\%%~ni.csproj
)
...
echo.
echo   ^<ItemGroup^>
echo     ^<Compile Include="..\..\%1.cs" Link="%1.cs" /^>
setlocal
set inclist=.
...
echo.
echo   ^<ItemGroup^>
echo       ^<PackageReference Include="Pr22" Version="2.1.0" /^>
echo   ^</ItemGroup^>
echo.
echo   ^<ItemGroup^>
...
echo     ^<Compile Include="..\..\Pr22.Extension\%incfile%.cs" Link="%incfile%.cs" /^>
)
...
for %%i in ( %PROJECTS% ) do (
	...
	set "list=!list!!guid!"
	echo Project^("{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}"^) = "%%i", "vcs160\%%i\%%i.csproj", "{!guid!}"
	echo EndProject
)
